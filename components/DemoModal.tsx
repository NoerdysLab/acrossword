...
function MiniTile({ letter, correct }: { letter: string; correct: boolean }) {
  return (
    <div
      ...
      style={{
        width: "2.75rem",
        height: "2.75rem",
        borderColor: correct ? "var(--accent)" : "var(--tile-border)",
        backgroundColor: correct ? "var(--accent)" : "var(--tile-bg)",
        color: correct ? "#ffffff" : "var(--text)",
      }}
    >
      {letter}
    </div>
        ...
        <div className="space-y-4 text-sm" style={{ color: "var(--text-secondary)" }}>
          ...
          <div className="rounded-lg p-4 space-y-3" style={{ backgroundColor: "var(--bg-secondary)" }}>
            ...
            <div className="flex gap-1.5 justify-center">
              {"ORBIT".split("").map((l, i) => (
                <MiniTile key={i} letter={l} correct />
              ))}
            </div>
